...
class Map:
    ...
    def fill_range_map(self):
        for map_range in self.ranges:
            for x in range(map_range.range_length):
                self.map[map_range.source_range_start + x] = (
                    map_range.destination_range_start + x
                )

    def get_num(self, source) -> int:
        for map_range in self.ranges:
            if (
                ...
                return map_range.destination_range_start + temp
        return source
...
def some_function(file_name: str = "input.txt", part_two: bool = False) -> int:
    ...
    for line in all_lines:
        ...
        if len(seeds) == 0 and line.strip().startswith("seeds: "):
            # seeds: 79 14 55 13
            ...
            while curr_spot != "END":
                for idx, map_item in enumerate(maps.keys()):
                    ...
                    if idx == len(maps) - 1:
                        curr_spot = "END"
                if curr_spot != "END":
                    temp_seed_val = curr_map_to_use.get_num(temp_seed_val)
