...
def some_function(file_name: str = "input.txt", part_two: bool = False) -> int:
    ...
    highest_calories = (
        (0, 0) if len(sorted_elf_to_calories) == 0 else sorted_elf_to_calories.pop(0)
    )
    print(f"Elf {highest_calories[0]} has {highest_calories[1]} calories.")
    return_me = highest_calories[1]

    if part_two:
        second_highest_calories = (
            (0, 0)
            if len(sorted_elf_to_calories) == 0
            else sorted_elf_to_calories.pop(0)
        )
        third_highest_calories = (
            (0, 0)
            if len(sorted_elf_to_calories) == 0
            else sorted_elf_to_calories.pop(0)
        )
